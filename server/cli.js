...
program
    ...
    .option('-c, --schedule <schedule>','Realm Configuration for sandbox start, stop action including time & days')
    .option('-j, --json','Formats the output in json')
    .action(function(options) {
        var realm = ( options.realm ? options.realm : null );
        if (!realm) {
        ...
        var asJson = ( options.json ? options.json : false );
        var schedule = ( options.schedule ? options.schedule : false );
        require('./lib/sandbox').cli.realm.update(realm, maxSandboxTTL, defaultSandboxTTL, schedule, asJson);
